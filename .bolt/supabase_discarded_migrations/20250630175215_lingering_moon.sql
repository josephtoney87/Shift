/*
  # Complete Database Schema Setup for CNC Shop Management

  1. New Tables
    - `users`: User management
    - `shifts`: Store shift information (A, B, C)
...
CREATE POLICY "Allow authenticated users to select shifts"
  ...
  FOR SELECT
  TO authenticated
  USING ((facility_id IS NULL) OR (facility_id = ((auth.jwt() ->> 'facility_id'::text))::uuid) OR ((auth.jwt() ->> 'facility_id'::text) IS NULL));

CREATE POLICY "Allow authenticated users to insert shifts"
  ON shifts
  FOR INSERT
  TO authenticated
  WITH CHECK ((auth.uid() IS NOT NULL) AND ((facility_id IS NULL) OR (facility_id = ((auth.jwt() ->> 'facility_id'::text))::uuid) OR ((auth.jwt() ->> 'facility_id'::text) IS NULL)));

CREATE POLICY "Allow authenticated users to update shifts"
  ON shifts
  FOR UPDATE
  TO authenticated
  USING ((auth.uid() IS NOT NULL) AND ((facility_id IS NULL) OR (facility_id = ((auth.jwt() ->> 'facility_id'::text))::uuid) OR ((auth.jwt() ->> 'facility_id'::text) IS NULL)))
  WITH CHECK ((auth.uid() IS NOT NULL) AND ((facility_id IS NULL) OR (facility_id = ((auth.jwt() ->> 'facility_id'::text))::uuid) OR ((auth.jwt() ->> 'facility_id'::text) IS NULL)));
...
CREATE POLICY "workers_facility_policy"
  ...
  FOR SELECT
  TO authenticated
  USING (((facility_id IS NULL) OR (facility_id = ((auth.jwt() ->> 'facility_id'::text))::uuid)) AND (deleted_at IS NULL));
...
CREATE POLICY "tasks_facility_policy"
  ...
  TO authenticated
  USING (((facility_id IS NULL) OR (facility_id = ((auth.jwt() ->> 'facility_id'::text))::uuid)) AND (deleted_at IS NULL))
  WITH CHECK (facility_id = ((auth.jwt() ->> 'facility_id'::text))::uuid);
