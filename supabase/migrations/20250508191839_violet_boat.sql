/*
  # Add Audit Trail, Soft Delete, and Facility Scoping

  1. New Tables
    - audit_logs: Tracks all changes to main tables
      - id (uuid, primary key)
      - table_name (text)
      - record_id (uuid)
      - operation (text)
  ...
  2. Changes
    - Added soft delete support (deleted_at) to main tables
    - Added facility scoping (facility_id) to main tables
    - Added unique constraints for parts and workers
    - Created audit triggers for main tables
    - Created materialized view for shift summary
    - Updated RLS policies to include facility scoping

  3. Security
    - Enabled RLS on audit_logs
    - Updated policies to include soft delete checks
*/
